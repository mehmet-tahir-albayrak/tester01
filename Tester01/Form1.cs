...
namespace Tester01
{
    public partial class Form1 : Form
    {
        ...
        int GetEventsLogs(out JsonEventLog[] logs)
        {
            ...
            ControlRichTextbox.Text = " \n EVENT LOGLAR ALINIYOR \n";
            ...
            try
            {
                ...
                try
                {
                    ...
                    ControlRichTextbox.Text += ("\n ALINAN LOG SAYISI " + logs.Length.ToString()+"\n");
                        ...
                        {
                            {
                                for (int i = 0; i < length; i += DATA_LOG_ENTRY_LEN)
                                {
                                    ...
                                    ControlRichTextbox2.Text += ("\n GÖNDERİLECEK DATA LOG SAYISI :  " + (logs.Length).ToString() + "\n");
                ...
                while (true)
                {
                    ...
                    if (VaccPort.BytesToRead > 0)
                    {
                        if (GetResponse(VaccPort, Commands.UsbCmd, Commands.USB_GET_LOG_TABLE, out rsp, out length))
                        {
                            ...
                            ControlRichTextbox.Text += ("\n GÖNDERİLEN LOG SAYISI :  " + (length / 6).ToString());
                            timeout = 0;
                        }
                    }
                    if (timeout++ > 10)
        ...
        {
            Thread th5 = new Thread(JsonPost2);
            th5.Start();
        ...
        private void JsonPost2()
        {
            ...
            ControlRichTextbox.Text += (" \n GÖNDERİLMEYE HAZIR LOG SAYISI : " + jsonDatas.Length.ToString() + "\n");
            progressBar1.Value = 25;
            System.Threading.Thread.Sleep(500);
            try
            {
                ControlRichTextbox.Text += " \n EVENT LOGLAR GÖNDERİLİYOR... \n";
                progressBar1.Value = 49;
                string access_token = getToken();
                int balance = jsonDatas.Length % 1;
                int j = ((jsonDatas.Length / 1) + (balance > 0 ? 1 : 0));
                for (int i = 0; i < j; i++)
                {
                    JsonEventLog[] logs = new JsonEventLog[i == j - 1 ? balance : 1];
                    Array.Copy(jsonDatas, i * 1, logs, 0, i == j - 1 ? balance : 1);
                    string strData = JsonConvert.SerializeObject(logs);
                    dataLogTxtBox.AppendText(strData);
                    byte[] datas = Encoding.ASCII.GetBytes(strData);
                    if (strData.Length != 0)
        ...
        private void JsonDataPost2()
        {
            JsonDataLog[] jsonDatas;
            int iLogLength = GetDataLogs(out jsonDatas);
            ControlRichTextbox2.Text += (" \n GÖNDERİLMEYE HAZIR DATA LOG SAYISI :  " + jsonDatas.Length + "\n");
            progressBar2.Value = 25;
            try
            {
                ControlRichTextbox2.Text += " \n DATA LOGLAR GÖNDERİLİYOR... \n";
                ...
                for (int i = 0; i < j; i++)
                {
                    JsonDataLog[] logs = new JsonDataLog[i == j - 1 ? balance : 1];
                    Array.Copy(jsonDatas, i * 1, logs, 0, i == j - 1 ? balance : 1);
                    for (int n = 0; n < logs.Length; n++)
                    {
                        string strData = JsonConvert.SerializeObject(logs[n]);
            ...
            ControlRichTextbox2.Text += " \n DATA LOGLAR GÖNDERİLDİ! \n";
            progressBar2.Value = 50;
        }

        private void DataLogSenkronizasyon_Click(object sender, EventArgs e)
        {
            Thread th4 = new Thread(JsonDataPost2);
            th4.Start();
        }
    }
}
